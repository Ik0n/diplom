        ...
        <div class="row">
            <div class="col-md-8 col-md-offset-2">
                <div class="panel panel-default">
                    <div class="panel-heading">{{trans('Друзья')}} / <a href="{{ route('users.myInboxRequestToFriend', ['user' => $authUser]) }}">Входящие заявки в друзья</a> /
                        <a href="{{ route('users.myOutboxRequestToFriend', ['user' => $authUser]) }}">Исходящие заявки в друзья</a></div>
                    <div class="panel-body">
                        @if($authUser->inboxFriends->count() >= 1 or $authUser->outboxFriends->count() >= 1)
                            @foreach($authUser->inboxFriends as $friend)
                        ...
                        @endforeach
                                @foreach($authUser->outboxFriends as $friend)
                                        <div class="text-center">
                                            <hr>
                                            <table class="message text-left">
                                                <tr class="autor-message"><td rowspan="6">
