...
class UserController extends Controller
{
    ...
    public function update(UserRequest $request, $id) {
        $attributes = $this->service->update($request,$id);
        return redirect(route('users.show.user', [
    ...
    public function myInboxRequestToFriend() {
        $attributes = $this->service->myInboxRequestToFriend();
        return view('layouts.users.index',[
            'users' => $attributes['users'],
            'authUserName' => $attributes['authUserName'],
            'authUser' => $attributes['authUser'],
    ...
    public function myOutboxRequestToFriend() {
        $attributes = $this->service->myOutboxRequestToFriend();
        return view('layouts.users.index',[
            'users' => $attributes['users'],
            'authUserName' => $attributes['authUserName'],
            'authUser' => $attributes['authUser'],
    ...
    */
    ...
    public function requestToBeFriends(User $user, Request $request) {
        $attributes = $this->service->requestToBeFriends($user,$request);
        return redirect(route('users.show.user' ,[
            'user' => $user
        ]));
    }
    ...
    public function deleteRequestToBeFriends(User $user) {
        $attributes = $this->service->deleteRequestToBeFriends($user);
        return redirect(route('users.show.user' ,[
            'user' => $attributes['user']
        ]));
    }
    ...
    public function subscribe(User $user) {
        $attributes = $this->service->subscribe($user);
        return redirect(route('users.show.user') ,[
            'user' => $user
        ]);
    }
    ...
    public function unsubscribe(User $user) {
        $attributes = $this->service->unsubscribe($user);
        return redirect(route('users.show.user') ,[
            'user' => $user
        ]);
    }
    ...
    public function addToFriendList(User $user) {
        $attributes = $this->service->addToFriendList($user);
        return redirect(route('users.show.user' ,[
            'user' => $attributes['user']
        ]));
    }
    ...
    public function deleteFromFriendList(User $user) {
        $attributes = $this->service->deleteFromFriendList($user);
        return redirect(route('users.show.user' ,[
            'user' => $attributes['user']
        ]));
    }
    ...
    */
    ...
    public function myPhotoAlbum(User $user) {
        $attributes = $this->service->myPhotoAlbum($user);
        return view('layouts.users.myPhotoAlbum', [
    ...
    public function storePhotoToAlbum(User $user, Request $request) {
        $attributes = $this->service->storePhotoToAlbum($user, $request);
        return redirect(route('users.myPhotoAlbum', ['user' => $user]));
    }

    public function myVideoAlbum(User $user) {
        $attributes = $this->service->myVideoAlbum($user);
        return view('layouts.users.myVideoAlbum', [
            'albums' => $attributes['albums'],
            'user' => $attributes['user'],
            'authUserName' => $attributes['authUserName']
        ]);
    ...
    public function storeVideoToAlbum(User $user, Request $request) {
        $attributes = $this->service->storeVideoToAlbum($user, $request);
        return redirect(route('users.myVideoAlbum', ['user' => $user]));
    }

    public function shareMessage(User $user, $message) {
        $attributes = $this->service->shareMessage($user,$message);
        return redirect(route('users.show.user', ['user' => $attributes['user']]));
    }

    public function myGroups(User $user) {
        $attributes = $this->service->myGroups($user);
        return view('layouts.users.myGroups', ['user' => $user, 'authUserName' => Auth::user()['name'], 'authUser' => Auth::user()]);

    }

    public function groupCreate(User $user) {
        $attributes = $this->service->groupCreate($user);
        return view('layouts.users.groupCreate', ['user' => $user, 'authUserName' => Auth::user()['name']]);
    }

    public function storeGroup(User $user, Request $request) {
        $attributes = $this->service->storeGroup($user, $request);
        return view('layouts.users.myGroups', ['user' => $user, 'authUserName' => Auth::user()['name'], 'authUser' => Auth::user()]);
    }
